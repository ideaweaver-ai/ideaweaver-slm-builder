...
function buildPythonConfig(cfg: Config, params: ReturnType<typeof estimateParams>) {
  const types = layerTypes(cfg);
  ...
  for (let i = 0; i < types.length; i += 4) {
    typesLines.push(
      "        " +
        types
          .slice(i, i + 4)
          .map((t) => `"${t === "full" ? "full_attention" : "sliding_attention"}",`)
          .join(" ")
    );
  }

  return `# Generated by IdeaWeaver SLM Builder (ideaweaver.ai)
# Architecture: Gemma-4-Nano style — interleaved local/global attention,
# grouped-query attention, QK-RMSNorm, partial RoPE, cross-layer KV sharing.
# Estimated parameters: ${params.total.toLocaleString()} (~${fmtParams(params.total)})
...
SLM_CONFIG = {
    ...
    "qk_norm": ${cfg.qkNorm ? "True" : "False"},
    "attention_k_eq_v": ${cfg.attentionKEqV ? "True" : "False"},

    "rope_local_base": ${cfg.ropeLocalBase.toFixed(1)},
    "rope_base": ${cfg.ropeBase.toFixed(1)},
    "partial_rotary_factor": ${cfg.partialRotaryFactor},

    "sliding_window": ${cfg.slidingWindow},
    "num_kv_shared_layers": ${cfg.numKvSharedLayers},
    "ple_dim": ${cfg.pleDim},
    "final_logit_softcapping": ${cfg.finalLogitSoftcapping || "None"},

    "dtype": torch.${cfg.precision},
    "layer_types": [
${typesLines.join("\n")}
    ],
}
...
total_tokens = ${cfg.totalTokens}

# ── Training hyperparameters ──
learning_rate = ${cfg.learningRate}
max_iters = ${cfg.maxIters}                 # optimizer steps (post grad-accum)
warmup_steps = ${cfg.warmupSteps}
min_lr = ${cfg.minLr}
eval_interval = ${cfg.evalInterval}
eval_iters = ${cfg.evalIters}
batch_size = ${cfg.batchSize}
block_size = ${cfg.blockSize}
gradient_accumulation_steps = ${cfg.gradAccumSteps}
weight_decay = ${cfg.weightDecay}
grad_clip_norm = ${cfg.gradClip}
adam_betas = (${cfg.beta1}, ${cfg.beta2})
adam_eps = ${cfg.adamEps}

# target hardware: ${cfg.gpu} (${GPU_INFO[cfg.gpu].arch}, ${GPU_INFO[cfg.gpu].vramGB} GB)
`;
}
...
export default function SLMBuilder() {
  ...
  const exportPython = () => download("slm_config.py", buildPythonConfig(cfg, params), "text/x-python");
  const exportJson = () => download("slm_config.json", JSON.stringify(cfg, null, 2), "application/json");
  const [copied, setCopied] = useState(false);
  const copyPython = async () => {
    await navigator.clipboard.writeText(buildPythonConfig(cfg, params));
    setCopied(true);
      ...
      <div className="border-b border-white/[0.06] px-6 py-12">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-3 max-w-2xl text-sm text-zinc-400 leading-relaxed">
            ...
            then export a ready-to-train Python config. Same architecture we teach in{" "}
            ...
            <div className="sticky top-24 space-y-4">
              <Card title="Run" subtitle="Live estimates from your config" icon="📊">
                ...
                <p className="mt-2 text-center text-[10px] text-zinc-600">
                  Preview only — export the config below to train for real.
                </p>
              </Card>

              <Card title="Export" subtitle="Take this config to a real training run" icon="⬇️">
                <div className="space-y-2">
                  <button
                    onClick={exportPython}
                    className="w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 py-2 text-left text-xs font-semibold text-white transition hover:border-violet-500/40"
                  >
                    Download slm_config.py
                  </button>
                  <button
                    onClick={exportJson}
                    className="w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 py-2 text-left text-xs font-semibold text-white transition hover:border-violet-500/40"
                  >
                    Download slm_config.json
                  </button>
                  <button
                    onClick={copyPython}
                    className="w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 py-2 text-left text-xs font-semibold text-white transition hover:border-violet-500/40"
                  >
                    {copied ? "Copied ✓" : "Copy Python config"}
                  </button>
                </div>
              </Card>
            </div>
